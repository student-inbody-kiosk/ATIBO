...
2022년 정부는 초중고생 체력증진을 목표로 학교체육 활성화를 추진했습니다. ATIBO는 초중고 체력단련실 내의 키오스크 및 관리자 페이지를 통해 학생들의 인바디 정보를 기록하고 간편한 출결 관리 서비스를 제공하는 웹 서비스입니다. <mark style="background-color:#fff5b1">로컬 컴퓨터에 **도커가 설치되어 있다면**, 실행 환경에 구애받지 않고 **누구나 ATIBO 웹 서비스를 다운받아 제공**할 수 있습니다.</mark>

처음 프로그램을 실행하면 예시 데이터가 존재합니다. <b>30명 학생(1학년 1반 1번 ~ 30번)</b>에 대한 <b>1년치 출결 & 인바디 데이터(2022년 7월 26일 ~ 2023년 7월 25일)</b>로 이뤄져 있습니다. 예시 데이터를 일괄 삭제하려면, '관리자 페이지 > 학생 관리 > 1학년 1반 조회 > 데이터 삭제'를 진행합니다. [관리자 계정 정보](#️-계정-권한)와 [예시 화면](#관리자-페이지)은 링크를 참조해주세요.
...
```
선생님(일반 계정)은 다음과 같은 기능을 이용할 수 있습니다.
...
- 반드시 ATIBO 웹서비스를 제공할 컴퓨터의 **공인 IP주소(`DOMAIN_OR_PUBLIC_IP`)와 포트 번호(`PORT`)를** 입력해야 합니다.
- 보안을 위해 데이터베이스 비밀번호(`DB_PASSWORD`) 수정을 권장합니다.
- 이메일을 통한 비밀번호 초기화 기능을 이용하기 위해서는 이메일(`EMAIL_HOST_USER`)과 이메일 비밀번호(`EMAIL_HOST_PASSWORD`)를 입력해주세요. 단 STMP을 통한 원격 연결이 허용된 이메일이어야 합니다
